<template>
  <div class="dashboard-page">
    <div class="header-action">
      ...
      <div class="header-buttons">
        <NuxtLink :to="`/admin/events/${eventId}/tickets`" class="btn-outline"
          >Ke Daftar Pendaftar</NuxtLink
        >
        <NuxtLink to="/admin/events" class="btn-outline"
          >Kembali ke Event</NuxtLink
        >
      </div>
    </div>
    ...
    <div v-else class="dashboard-content mt-4">
      ...
      <div class="metrics-grid">
        ...
        <div class="metric-card glass-panel">
          ...
          <div class="metric-info">
            ...
            <div class="metric-value">
              {{ metrics.total }}
              <span v-if="event.quota" class="quota-text"
                >/ {{ event.quota }}</span
              >
            </div>
        ...
        <div v-if="event.isPaid" class="metric-card glass-panel">
          ...
          <div class="metric-info">
            ...
            <p class="metric-desc">
              Dari {{ metrics.approved }} tiket approved
            </p>
          </div>
        </div>
      </div>
      ...
      <div v-if="event.isEventPeriod" class="mt-6">
        <div class="dashboard-card glass-panel chart-container">
          <h2 class="section-title">Tren Kehadiran Pendaftar</h2>
          <div class="chart-scroll-wrapper">
            <div
              :style="{ minWidth: attendanceChartMinWidth }"
              class="chart-inner-canvas"
            >
              <Line
      ...
      <div class="dashboard-grid mt-6">
        ...
        <div class="dashboard-card glass-panel">
          ...
          <div
            v-if="!recentRegistrations.length"
          ...
          <div v-else class="recent-list">
            <div
              v-for="ticket in recentRegistrations"
              :key="ticket.id"
              class="recent-item"
            >
...
const recentRegistrations = computed(
  () => response.value?.recentRegistrations || [],
);
...
const trendChartData = computed(() => {
  if (!response.value?.regTrend) return null;
  return {
    labels: response.value.regTrend.labels,
    datasets: [
      {
        ...
        fill: true,
        tension: 0.4,
        data: response.value.regTrend.data,
      },
    ],
  };
});
...
const attendanceChartData = computed(() => {
  if (!response.value?.attendanceTrend) return null;
  return {
    labels: response.value.attendanceTrend.labels,
    datasets: [
      {
        label: "Masuk",
        borderColor: "#4ade80",
        ...
        tension: 0.4,
        fill: false,
        data: response.value.attendanceTrend.masuk,
      },
      {
        label: "Keluar",
        borderColor: "#facc15",
        ...
        tension: 0.4,
        fill: false,
        data: response.value.attendanceTrend.keluar,
      },
      {
        label: "Belum Hadir",
        borderColor: "#94a3b8",
        ...
        fill: false,
        borderDash: [5, 5],
        data: response.value.attendanceTrend.belumHadir,
      },
    ],
  };
});
...
const attendanceChartMinWidth = computed(() => {
  const labels = response.value?.attendanceTrend?.labels || [];
  if (labels.length <= 12) return "100%";
  return `${labels.length * 40}px`; // Each hour gets 40px width
});
</script>
